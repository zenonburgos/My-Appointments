...
class ChartController extends Controller
{
    ...
    {	
    	// created_at -> dateTime
    	//dd(); //encerrar lo siguiente en el DD si desea ver resultado en pantalla sin monthlyCounts
		$monthlyCounts = Appointment::select(
			DB::raw('MONTH(created_at) as month'), 
			DB::raw('COUNT(1) as count')
		)->groupBy('month')->get()->toArray();
		// [ ['month'=>6 (junio), 'count'=>3]]
		// [0, 0, 0, 0, 0, 0, 3, 0, 0, 0, 0, 0, 0]

		$counts = array_fill(0, 12, 0); //index, qty, value
		foreach ($monthlyCounts as $monthlyCount) {

			$index = $monthlyCount['month']-1; //recordar que los elementos en un array empiezan con 0, por eso el -1 para que de el número de mes exacto
